...
import { X, Users, Coins, Check, AlertCircle, FolderKanban } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await onSubmit({
        ...
        tanggal,
        keterangan: keterangan || (currentKat ? `${currentKat.nama_kategori}` : 'Iuran Lain')
      });
      setSelectedAnggotaIds([]);
      onClose();
    } catch (err: any) {
      setErrorMsg(err.message || 'Gagal menyimpan iuran massal.');
    } finally {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white">
                Input Iuran Lain Massal (Multi-Select)
              </h2>
              <p className="text-xs text-slate-500">
                Pencatatan iuran khusus kegiatan & acara <em>(tanpa acuan bulan)</em>
              </p>
            </div>
          </div>
          <button
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">
                Pilih Kategori Iuran <span className="text-slate-500">*</span>
              </label>
              <select
            ...
            <div>
              ...
              <input
                type="text"
                placeholder="cth: Pembayaran tahap 1"
                value={keterangan}
          ...
          <div className="p-4 rounded-xl bg-purple-50 dark:bg-purple-950/40 border border-purple-200 dark:border-purple-800 flex items-center justify-between">
            <div>
              ...
              <p className="text-[11px] text-purple-600/80">
                Kategori: <strong>{currentKat?.nama_kategori || 'Iuran Lain'}</strong>
              </p>
            </div>
