...
export default function App() {
  ...
  // Multi-Select Iuran Lain Submit
  const handleAddIuranLainBatch = async (data: {
    ...
    await api.addTransaksiLainBatch(data);
    showToast(`Berhasil mencatat iuran untuk ${data.anggota_ids.length} anggota! (Tersimpan otomatis)`);
    await loadAllData(true);
  };
  ...
  const defaultNominalKas = Number(settings.nominal_kas_bulanan) || 20000;
